...
function getGeneralData() {
	...
	$('title').html(isDefaultLanguage ?
					'Jonas Souza | Currículo' :
					'Jonas Souza | Curriculum');
	...
	$('nav').attr('aria-label', isDefaultLanguage ?
								'Menu de Navegação' :
								'Navigation Menu');
	$('.brand').attr('aria-label', isDefaultLanguage ?
									'Logotipo do Jonas Souza' :
									'Jonas Souza Logo');
	$('#navbarToggler').attr('aria-label', isDefaultLanguage ?
											'Botão Menu de Navegação' :
											'Navigation Menu Button');
	$('#btnLanguage').find('img').attr('alt', isDefaultLanguage ?
												'Versão em Português' :
												'Version in English');

	// APRESENTATION                                                        
	$('.box').attr('aria-label', isDefaultLanguage ?
								'Imagem de fundo de tecnologia nas cores laranja, preto e roxo' :
								'Technology background image in orange, black and purple');
	$('.apresentation').find('img').attr('alt', isDefaultLanguage ?
												'Imagem de Jonas Souza' :
												'Image of Jonas Souza');
	$('.apresentation-title').html(isDefaultLanguage ?
									'Desenvolvedor Web' :
									'Web Developer');
	...
	$('.about-menu').find('.card-header').find('div').html(isDefaultLanguage ?
															'Sobre mim' :
															'About me');
	...
	$('#btnBackToTop').attr('aria-label', isDefaultLanguage ?
											'Voltar para o topo' :
											'Back to the top');
...
function getTitle(page) {
	let title = $('#' + page).find('.item-title').html();
...
function getEducation(objJSONDataPage, page) {
	...
	for (let i in objJSONDataPage) {
		...
		aboutContent.appendChild(elementData('p', pageItem.degree + ", " + 
													pageItem.fieldOfStudy, classTitle2));
		...
		aboutContent.appendChild(elementData('p', pageItem.description, classDescription));
	}
}
...
function getSkills(objJSONDataPage, page) {
	...
	for (let i in objJSONDataPage) {
		...
		for (let l in pageItem.languages) {
			...
			let p = elementData('p', languages.language + ': ' + 
										languages.proficiency, classTitle2);
			divLanguages.appendChild(p);
		}

		let divMarkupProgramming = elementData('div', '', 'devicon-box');
		for (let c in pageItem.markupProgramming) {
			divMarkupProgramming.appendChild(skillsIcon(pageItem.markupProgramming[c]));
		}
...
function getCertification(objJSONDataPage, page) {
	...
	let div = elementData('div', '', 'd-flex justify-content-around certificationLegend');
	...
	for (let i in objJSONDataPage) {
		...
		let icon = pageItem.degreeType == 'certification' ? 
					...
					pageItem.degreeType == 'graduation' ? 
											'fas fa-graduation-cap' : 'fas fa-certificate';
		let certificationItemDiv = elementData('div', '', 'd-flex align-items-center certificationItem');
